...
import { Plus, Minus, Trash2, Send, ChevronLeft, TableIcon, ShoppingCart } from "lucide-react";
import { MenuItem, OrderItem, Order, MENU_ITEMS, TABLES, SessionUser } from "../types";
...
export function WaiterDashboard({ user, onSubmitOrder, onBack }: Props) {
    ...
    return (
      <div className="min-h-screen bg-stone-900 flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-stone-400 mb-8">Total: <span className="text-amber-400">${total.toFixed(2)}</span></p>
    ...
    <div className="min-h-screen bg-stone-900 flex flex-col">
      ...
      <div className="bg-stone-800 border-b border-stone-700 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-white">Registrar Pedido · Mesa</h1>
            <p className="text-amber-400/70 text-xs">RF-01 · {user.name}</p>
          </div>
        </div>
        <button
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!selectedTable ? (
            <div className="p-6">
              <h2 className="text-white mb-2">Seleccionar Mesa</h2>
              <p className="text-stone-400 text-sm mb-6">Elija la mesa para registrar el pedido</p>
              <div className="grid grid-cols-4 gap-3">
                {TABLES.map((t) => (
                  <button
                    key={t}
                    onClick={() => setSelectedTable(t)}
                    className="bg-stone-800 hover:bg-amber-500/20 hover:border-amber-500 border border-stone-700 rounded-xl p-4 flex flex-col items-center transition-all"
                  >
                    <TableIcon size={20} className="text-stone-400 mb-1" />
            ...
            <div className="flex flex-col">
              ...
              <div className="px-4 py-3 bg-amber-500/10 border-b border-amber-500/20 flex items-center gap-2">
                ...
                <span className="text-amber-300">Mesa {selectedTable} seleccionada</span>
                <button
        ...
        {showCart && (
          <div className="w-80 bg-stone-800 border-l border-stone-700 flex flex-col">
            <div className="p-4 border-b border-stone-700">
              <h3 className="text-white">Pedido · Mesa {selectedTable}</h3>
