class CocinaView{
    ...
    render(pedidos){
        ...
        contenedor.innerHTML = estados.map(estado => {
            let pedidosEstado = pedidos.filter(pedido => pedido.estado === estado.key);

            if (estado.key === "ENTREGADO") {
                const ahora = new Date();
                pedidosEstado = pedidosEstado.filter(pedido => {
                    if (!pedido.updatedAt) return true;
                    const diferenciaMinutos = (ahora - new Date(pedido.updatedAt)) / 1000 / 60;
                    return diferenciaMinutos < 2; // Desaparece a los 2 minutos
                });
            }
